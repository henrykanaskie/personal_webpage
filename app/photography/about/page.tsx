...
// ─── Photo frame (about page previews: fixed tile so they fit together) ─────────
...
function PhotoFrame({
  ...
  const [r, g, b] = accent;
  const filterId = `grain-about-${sectionId}-${index}`;

  const [wRatio, hRatio] = photo.ratio.split("/").map(Number);
  const ratio = wRatio / hRatio;
  // Same height for all so the row fits nicely; width varies by aspect ratio
